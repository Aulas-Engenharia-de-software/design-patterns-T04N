...
function App() {
  ...
  function setStrategy(key: AvailableOptions) {
    if (!operatorNumber && !operatedNumber) {
      return;
    } else if (operatorNumber && operatedNumber && key !== "restDivision") {
      calculate(true);
    }
  ...
  function calculate(calculateSettingOperation: boolean) {
    ...
    switch (context.current.curretOperation) {
      case "%":
        result = context.current.getPercentage(+operatedNumber, operatorNumber.includes("%") ? +operatorNumber.replace(/[\s%]/g, "") / 100 : +operatorNumber);
      break;
      case "^":
        result = context.current.getPow(+operatedNumber, +operatorNumber);
      break;
      case "√":
        result = context.current.getSquareRoot(+operatedNumber, +operatorNumber);
      break;
      case "/":
        result = context.current.getDivision(+operatedNumber, +operatorNumber);
      break;
      case "+":
        result = context.current.getSum(+operatedNumber, +operatorNumber);
      break;
      case "-":
        result = context.current.getSub(+operatedNumber, +operatorNumber);
      break;
      case "*":
        result = context.current.getMultiplication(+operatedNumber, +operatorNumber);
      break;
    }
