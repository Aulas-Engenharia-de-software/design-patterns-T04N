...
function App() {
  ...
  const decorator = useRef<Calculator>(new InitialCalculatorDecorator());
  ...
  function setStrategy(key: AvailableOptions) {
    ...
    const newDecorator = operations[key];
    
    if (!newDecorator) return;
    
    decorator.current = newDecorator;
    
    setOperation(decorator.current.operation);
  ...
  function calculate(calculateSettingOperation: boolean) {
    ...
    const result = decorator.current.calculate(formatNumbers(operatedNumber), formatNumbers(operatorNumber));

    decorator.current = operations.initialCalculatorDecorato;
